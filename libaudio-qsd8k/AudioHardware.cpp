...
static status_t do_route_audio_dev_ctrl(uint32_t device, bool inCall, uint32_t rx_acdb_id, uint32_t tx_acdb_id)
{
    ...
    if (device == SND_DEVICE_HANDSET) {
           ...
           out_device[0] = SPKR_PHONE_STEREO;
           ...
           out_device[0] = HANDSET_SPKR;
           mic_device[0] = ADSP_AUDIO_DEVICE_ID_HANDSET_DUAL_MIC;
           mic_device[1] = MIKE_ID_HANDSET;
           LOGD("Handset with DualMike");
    } else if (device == SND_DEVICE_SPEAKER_DUAL_MIC) {
           out_device[0] = SPKR_PHONE_MONO;
           mic_device[0] = ADSP_AUDIO_DEVICE_ID_SPKR_PHONE_DUAL_MIC;
...
status_t AudioHardware::doAudioRouteOrMute(uint32_t device)
{
    ...
    return do_route_audio_dev_ctrl(device, mMode == AudioSystem::MODE_IN_CALL, 0, 0);
}
...
ssize_t AudioHardware::AudioStreamInMSM72xx::read( void* buffer, ssize_t bytes)
{
    ...
    if (mFormat == AudioSystem::AAC) {
       if (bytes < 512) {
        LOGE("Error, the buffer size passed is not compatible %d", (int) bytes);
        return -1;
       }
    }
    bytes = 0;
